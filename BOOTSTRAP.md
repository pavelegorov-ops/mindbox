...
с бизнес-данными в подкаталог `private/usmall-mindbox/`:

```bash
git clone <URL приватного репо Usmall> private/usmall-mindbox
```

Имя подкаталога должно совпадать с именем приватного репозитория —
движок сам находит активного тенанта как единственный подкаталог
`private/` (кроме `_template/`).
