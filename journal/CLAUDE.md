...
**Дефолт — полнотекстовый BM25-поиск `journal_search.py`. Начинай с него,
а не с грепа.** Он ищет по абзацам тела статьи и матчит RU↔EN
(`удержание` ↔ `retention`), поэтому достаёт факты, которые не попали в
карточки `summary_ru` / `key_points` / `fact_index.json` — а таких много,
и триаж по summary их молча теряет. Прогоняй **несколько переформулировок**
под разные механики: одна выдача корпус не исчерпывает.

Запускай **интерпретатором из `.venv/` репо mindBox, не системным
`python`** — скрипт зависит от `rank_bm25` / `pymorphy3`, которых в
системном питоне нет:

```bash
# из корня репо mindBox:
.venv/Scripts/python.exe scripts/journal_search.py "удержание клиентов"                     # Windows
.venv/bin/python         scripts/journal_search.py "open rate выросла на 30" --section cases # macOS/Linux
```

Голый `python scripts/journal_search.py …` падает с
`ModuleNotFoundError: rank_bm25` — это **не «поиск сломан», а не тот
интерпретатор**. Выдача — JSON со score, slug, `section_title` (ближайший
H2/H3) и сниппетом; по нему решаешь, какие страницы открыть целиком. Индекс
— `journal/<section>/search_index.pkl`, собирается при `sync.py` (или
вручную `python scripts/build_bm25.py`).

**Fallback — греп-триаж, если BM25 использовать не получится.** BM25
недоступен, когда индекса нет (`search_index.pkl` отсутствует — свежий
клон, не прогнаны `sync.py` / `build_bm25.py`) или venv не собран. Тогда
работай по слоям ниже — старый триаж по-прежнему даёт рабочий ответ. Их же
используй **параллельно** с BM25 для faceted-обзоров («дай 3 кейса по
механике X»): тут структурированные индексы удобнее полнотекста.
...
(Полнотекстовый BM25 — `journal_search.py` — вынесен наверх раздела как
дефолтный первый шаг; сюда, в триаж по summary/индексам, откатывайся
только если BM25 недоступен либо нужен faceted-обзор.)
