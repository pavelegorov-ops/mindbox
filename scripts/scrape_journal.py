...
Output layout (under <--out>/<--section>/, e.g. `journal/education/`):
...
Onboarding documentation for Claude Code (`journal/CLAUDE.md`) is hand-maintained
at the journal root and not regenerated by this scraper.

Internal links between journal articles are rewritten to local `<slug>.md` so
Claude can follow them via the Read tool. Only same-section links are rewritten;
cross-section ones (e.g. an education article linking to /journal/cases/) stay
absolute.
"""
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        default="journal",
        help="parent output directory; the scraper writes into <out>/<section>/ (default: journal)",
    )
    parser.add_argument(
        "--concurrency",
        type=int,
